...
function get_doc_primary_no($field,$table,$company_id)
{
	try
	{
        ...
        // START - Added by Sancheev

		/*$primary_rs = db_query("IFNULL( CONCAT( RIGHT(YEAR(NOW()),2),'-',  LPAD(  COUNT($field) + 1 ,$digit,REPEAT('0', $digit))), CONCAT( RIGHT(YEAR(NOW()),2),'-',  CONCAT( REPEAT('0', $digit-1),'1')    )) AS $field "
            , $table,"YEAR(created_date) = YEAR(NOW())",'1','0',$field,'desc');*/
       $primary_rs = db_query("IFNULL( CONCAT( RIGHT(YEAR(NOW()),2),'-',  LPAD(  SUBSTRING_INDEX($field,'-',-1) + 1 ,$digit,REPEAT('0', $digit))), CONCAT( RIGHT(YEAR(NOW()),2),'-',  CONCAT( REPEAT('0', $digit-1),'1')    )) AS $field "
            , $table,"YEAR(created_date) = YEAR(NOW())",'1','0','created_date','desc');

        // END - Added by Sancheev
...
function get_mail_signature_by_access_level($emp_access_id)
{
	try
	{
		...
		$rs					= db_query('name, office_email ,cms_master_employer.employer_name,cms_master_employer.mail_signature',
										'cms_employees INNER JOIN cms_master_employer on cms_employees.employer_id = cms_master_employer.id',
										'access_level IN (' . $c_emp_access_id. ') AND cms_employees.is_active = 1');
